# devenv module that bridges mcp-servers-nix's declarative module system
# with devenv's native claude.code.mcpServers configuration.
#
# Instead of generating .mcp.json via shellHook (like the flake-parts module),
# this outputs through devenv's built-in claude.code.mcpServers option.
{
  config,
  pkgs,
  lib,
  ...
}:
let
  mcp-lib = import ../lib;
  cfg = config.mcp-servers;

  evaluated = mcp-lib.evalModule pkgs {
    inherit (cfg) programs settings;
    flavor = "claude-code";
  };

  # settings.servers is freeform; it won't exist when no programs are enabled
  # and no custom servers are defined
  servers = evaluated.config.settings.servers or { };

  # devenv only supports "stdio" and "http" transport types
  mapType =
    type:
    if type == null then
      "stdio"
    else if type == "sse" then
      "http"
    else
      type;

  transformServer =
    _name: server:
    let
      serverType = server.type or null;
    in
    lib.filterAttrs (_: v: v != null && v != [ ] && v != { }) (
      {
        type = mapType serverType;
      }
      // lib.optionalAttrs (server ? command) {
        inherit (server) command;
      }
      // lib.optionalAttrs (server ? args && server.args != [ ]) {
        args = map toString server.args;
      }
      // lib.optionalAttrs (server ? env && server.env != { }) {
        env = lib.mapAttrs (_: toString) server.env;
      }
      // lib.optionalAttrs (server ? url) {
        inherit (server) url;
      }
      // lib.optionalAttrs (server ? headers && server.headers != { }) {
        inherit (server) headers;
      }
    );
in
{
  options.mcp-servers = {
    programs = lib.mkOption {
      type = lib.types.attrsOf lib.types.anything;
      default = { };
      description = ''
        MCP server program configurations.
        Uses the same interface as the flake-parts module.
      '';
      example = lib.literalExpression ''
        {
          playwright.enable = true;
          filesystem = {
            enable = true;
            args = [ "." ];
          };
        }
      '';
    };

    settings = lib.mkOption {
      type = lib.types.submodule {
        freeformType = (pkgs.formats.json { }).type;
      };
      default = { };
      description = ''
        Additional freeform configuration for MCP servers.
      '';
    };
  };

  config = lib.mkIf (servers != { }) {
    claude.code.mcpServers = lib.mapAttrs transformServer servers;
  };
}
